...
// --- SAYFA GEÇİŞ SARMALAYICI (Daha Stabil Expansion) ---
  ...
  useEffect(() => {
    ...
    const moveCursor = (e) => {
      if (cursorRef.current) {
        cursorRef.current.style.transform = `translate3d(${e.clientX - 10}px, ${e.clientY - 30}px, 0)`;
...
// --- HAREKETSİZLİK ZAMANLAYICISI (5 Dakika - Fixlendi) ---
          ...
          {isInfoOpen && (
            <Motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }} exit={{ opacity: 0 }} className="fixed inset-0 bg-black/60 z-[100] flex items-center justify-center p-10 backdrop-blur-sm">
              <Motion.div initial={{ scale: 0.8 }} animate={{ scale: 1 }} className="bg-[#f4f1ea] max-w-4xl p-16 rounded-[3rem] shadow-2xl relative border-l-[20px] border-[#c04a2e]">
                ...
                <div className="space-y-10 text-2xl text-gray-700 leading-relaxed text-left">
                  <p><strong>Hatice Koç Kimdir?</strong><br/>Bolvadin Hasan Gemici Ortaokulu ve Bolvadin BİLSEM resim öğretmeni.</p>
                  <p><strong>Neden İZ?</strong><br/>Bolvadin’in tarihine, doğasına ve mutfak kültürüne bırakılmış sanatsal bir izdir.</p>
                </div>
              </Motion.div>
            </Motion.div>
          )}
...
const VisitorBook = () => {
  ...
  const addMark = (e) => {
    const newMark = { id: Date.now(), x: e.clientX, y: e.clientY, emoji: emojis[Math.floor(Math.random() * emojis.length)] };
        ...
        <div className="bg-watercolor-content flex flex-col items-center w-full h-full justify-center">
          <div className="flex flex-row items-center gap-12 w-full max-w-[1600px] justify-between px-10">
            ...
            <div className="flex flex-col md:flex-row items-center gap-16 bg-white p-12 rounded-[3rem] shadow-2xl w-full border-[12px] border-white relative z-10 overflow-hidden">
                <div className="w-1/2 relative cursor-none group overflow-hidden rounded-2xl shadow-inner border border-gray-100 bg-gray-50" onMouseMove={handleMouseMove} onMouseLeave={() => setMagnifier({ ...magnifier, show: false })} onTouchMove={(e) => handleMouseMove(e.touches[0])} onTouchEnd={() => setMagnifier({ ...magnifier, show: false })}>
                  <img src={`/src/assets/gallery/${resimler[index].file}`} className="w-full max-h-[60vh] object-contain block select-none" alt={resimler[index].name} />
                  {magnifier.show && <div className="absolute pointer-events-none border-4 border-white shadow-2xl rounded-full z-50 bg-white" style={{ width: 250, height: 250, left: `${magnifier.x}%`, top: `${magnifier.y}%`, transform: 'translate(-50%, -50%)', backgroundImage: `url(/src/assets/gallery/${resimler[index].file})`, backgroundRepeat: 'no-repeat', backgroundSize: '250%', backgroundPosition: `${magnifier.x}% ${magnifier.y}%` }} />}
                </div>
                <div className="w-1/2 space-y-8">
                  ...
                  <p className="text-sm text-gray-400 uppercase tracking-[0.2em] font-medium pt-8">🔍 Resme dokunarak detayları inceleyin (Otomatik Akış: 10sn)</p>
                </div>
            </div>
          ...
          {resimler.map((img) => (
            <Link key={img.id} to={`/puzzle/${img.id}`}>
              <Motion.div whileHover={{ y: -10 }} className="bg-white p-5 rounded-[2.5rem] shadow-xl border-4 border-transparent hover:border-[#c04a2e] transition-all group">
                <div className="overflow-hidden rounded-2xl h-64"><img src={`/src/assets/gallery/${img.file}`} className="w-full h-full object-cover group-hover:scale-105 transition-transform" alt={img.name} /></div>
        ...
        {!solved && (
          <div className="fixed top-8 right-8 flex flex-col items-end gap-6 z-50">
            ...
            <div className="flex gap-4">
              {[3, 4, 5].map((size) => (
                <button key={size} onClick={() => changeDifficulty(size)} className={`px-6 py-2 rounded-full font-bold transition-all border-2 ${gridSize === size ? 'bg-[#c04a2e] text-white' : 'bg-white text-[#c04a2e]'}`}>{size}x{size}</button>
              ))}
            </div>
          </div>
        )}
              ...
              {solved && (
                <Motion.div initial={{ opacity: 0, scale: 0.8 }} animate={{ opacity: 1, scale: 1 }} className="flex flex-col items-start gap-8 max-w-md">
                  <div className="bg-white/80 p-10 rounded-[3rem] shadow-2xl border-l-[12px] border-[#c04a2e]">
                    ...
                    <p className="text-xl text-gray-500 italic leading-relaxed mb-10">Bolvadin'in bu eşsiz parçasını başarıyla bir araya getirdiniz. Hatice Koç'un fırçasından çıkan bu eser artık tam bir bütün.</p>
